...
class OpenCodeGoAdapter {
  ...
  /**
   * 图片请求计价（LlmAdapter 契约的一部分，可选语义 = 返回 undefined 表示本路由不声明图片计价）。
   *
   * 为什么必须显式实现：dsh-token-meter 的 measure() 会**无条件**调用
   * `llm.imageRequestPricing(provider, model)`，而 llm 服务内部是
   * `this.adapters.get(provider)?.adapter.imageRequestPricing(...)`——可选链只保护
   * "注册项不存在"，**不保护方法缺失**。dsh-compaction-basic 的 prepareCompaction()
   * 与 dsh-acp 都会走到这里，因此缺此方法会让使用本插件模型的会话在**上下文压缩时**
   * 抛 `TypeError: adapter.imageRequestPricing is not a function`。
   * 继承 dsh-llm 抽象类的适配器天然带一个返回 undefined 的默认实现；本插件刻意不依赖
   * dsh-llm 包（零依赖），所以这里显式补上等价实现。
   * @returns {undefined} 不声明图片计价，调用方回退到自身启发式估算。
   */
