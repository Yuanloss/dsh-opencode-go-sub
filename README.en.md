...
- **`x-opencode-session` header (mandatory for OpenCode Go since 2026-09-05)**: the gateway requires a stable per-conversation id on every inference request; missing it returns `400 MissingSessionID`. The plugin automatically reads the conversation id DSH's agent-loop injects (`GenerateOptions.sessionId`, shaped like `session-<uuid>`), normalizes it to the bare UUID, and sends it on every request; non-conversation direct calls fall back to a process-stable UUID, so no configuration is needed. If you reach opencode-go through DSH's built-in pi-ai route instead of this plugin, discussion [#5495](https://github.com/deepseek-ai/deepseek-harness/discussions/5495) offers a `sessionHeader: x-opencode-session` profile option.
